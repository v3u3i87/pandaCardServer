...
local _M = function(role,data)
	...
	if not open_config:check_level(role,open_config.need_level.soldier_awaken) then return 101 end

	local soldier = role.soldiers:get(data.id)
	if not soldier then return 500 end
	
	local canup = config:canawaken(soldier)
	if not canup then return 101 end
	
	local cost = config:get_awaken_cost(soldier)
