...
local _M = function(role,data)
	...
	local canup,add_exp = config:can_car_star_up(commander,data.cid)
	if not canup then return 101 end
	
	local cost = config:get_car_star_up_cost(commander,data.cid)
	...
	commander:car_star_up(data.cid,cost)
	return 0
end
