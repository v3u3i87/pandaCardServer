...
_M.class = "role.virtual"
_M.push_name  = "virtual"
_M.changed_name_in_role = "virtual"

_M.is_list = true
_M.is_key_num = true

function _M:on_time_up()
	if self.data[task_config.task_activity_id] then 
		self.data[task_config.task_activity_id] = nil	
		self:changed(task_config.task_activity_id)	
	end
end
...
function _M:gain(t,n)
	...
	self.role:virtula_add_count(t,n)
	self:changed(t)
	if t == task_config.task_activity_id then 
		self.role.tasklist:trigger(task_config.trigger_type.activity_value,self.data[t])
	end
	return self.data[t]
end
