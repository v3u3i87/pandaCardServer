...
ngx.log(ngx.ERR,"run 1")
	if self.global:get_status() ~= self.global.status_type.running then
		...
		if arg.gm ~= ngx.md5(self.global.gm_password) then
			...
			ngx.log(ngx.ERR,"server is not running",ok,err)
			return
		end
	end
	ngx.log(ngx.ERR,"run 2")
	self.global:init()
	ngx.log(ngx.ERR,"run 3")
...
ngx.log(ngx.ERR,"client init ",ok,err)
